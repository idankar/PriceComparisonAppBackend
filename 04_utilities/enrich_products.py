...
# --- REFINED PRODUCT_TYPE_MAP ---
# This version has been cleaned to remove the most ambiguous and duplicated keywords.
PRODUCT_TYPE_MAP = {
    ...
    "Toothbrush": ["מברשת שיניים", "מברשת חשמלית"],
    "Mouthwash": ["שטיפת פה", "מי פה"],
    "Soap": ["סבון", "סבון ידיים", "סבון נוזלי", "סבון מוצק", "סבון רחצה", "סבון אנטיבקטריאלי"],
    "Face Wash": ["סבון פנים", "ג'ל ניקוי", "תחליב ניקוי", "ניקוי פנים", "מוס ניקוי"],
    "Moisturizer": ["קרם לחות", "תחליב לחות", "קרם פנים", "קרם גוף"],
    "Sunscreen": ["קרם הגנה", "מקדם הגנה", "הגנה מהשמש"],
    "Vitamins": ["ויטמינים", "ויטמין", "מולטי ויטמין"],
    "Supplements": ["תוספי תזונה", "תוסף", "פרוביוטיקה", "קולגן"],
    "Diapers": ["חיתולים", "חיתול", "טיטולים"],
    "Baby Wipes": ["מגבונים", "מגבונים לחים", "מגבוני תינוק"],
    "Baby Formula": ["תמ״ל", "תרכובת מזון", "תחליף חלב", "פורמולה"],
    "Feminine Hygiene": ["תחבושות", "טמפונים", "פדים יומיים", "מוצרי היגיינה נשית"],
    "Contraceptives": ["אמצעי מניעה", "קונדומים", "גלולות"],
    "Pregnancy Test": ["בדיקת הריון"],
    "Thermometer": ["מדחום", "מד חום"],
    "Bandages": ["פלסטרים", "פלסטר", "תחבושות", "אגד"],
    "Antiseptic": ["חומר חיטוי", "פולידין", "סבידין"],
    "Hand Sanitizer": ["ג'ל אלכוהול", "חיטוי ידיים", "אלכוג'ל"],
    "Allergy Medicine": ["תרופה לאלרגיה", "אנטיהיסטמין"],
    "Cold Medicine": ["תרופה להצטננות"],
    "Cough Syrup": ["סירופ שיעול"],
    "Nasal Spray": ["ספריי לאף", "טיפות אף"],
    "Eye Drops": ["טיפות עיניים", "דמעות מלאכותיות"],
    "Contact Lens Solution": ["תמיסה לעדשות", "נוזל עדשות"],
    "Razor": ["סכין גילוח", "סכיני גילוח"],
    "Shaving Cream": ["קצף גילוח", "ג'ל גילוח", "קרם גילוח"],
    "After Shave": ["אפטר שייב"],
    "Hair Dye": ["צבע שיער"],
    "Nail Polish": ["לק", "לק ציפורניים"],
    "Nail Polish Remover": ["מסיר לק"],
    "Makeup Remover": ["מסיר איפור", "מים מיסלריים"],
    "Foundation": ["מייק אפ", "פאונדיישן", "מייקאפ"],
    "Mascara": ["מסקרה"],
    "Lipstick": ["שפתון", "ליפסטיק", "אודם"],
    "Eyeliner": ["אייליינר", "עיפרון עיניים"],
    "Acne Treatment": ["טיפול באקנה"],
    "Anti-Aging Cream": ["קרם אנטי אייג'ינג", "נגד קמטים"],
    "Serum": ["סרום", "סרום פנים"],
    "Face Mask Treatment": ["מסכת פנים", "מסכת בד", "מסכת חימר"],
    "Pain Reliever": ["משכך כאבים", "אקמול", "נורופן", "אופטלגין"],
    "Reading Glasses": ["משקפי קריאה"]
}
...
def get_products_to_enrich(cursor):
    cursor.execute("""
        SELECT product_id, canonical_name, brand FROM products
        WHERE attributes IS NULL OR attributes->>'product_type' IS NULL ORDER BY product_id;
    """)
...
def extract_attributes_with_rules(product_name: str):
    """
    Finds the best product_type by selecting the category with the longest matching WHOLE WORD keyword.
    """
    attributes = {}
    best_match_type = None
    longest_match_len = 0

    # Step 1: Find the best product_type by longest WHOLE WORD keyword match
    for p_type, keywords in PRODUCT_TYPE_MAP.items():
        for keyword in keywords:
            # This regex pattern ensures we match whole words only
    ...
    # Step 2: Call specialized parsers (if any)
    SPECIALIZED_PARSERS = {
        "Reading Glasses": _parse_reading_glasses
    }
    if best_match_type and best_match_type in SPECIALIZED_PARSERS:
        special_attributes = SPECIALIZED_PARSERS[best_match_type](product_name)
        attributes.update(special_attributes)

    # Step 3: Run the corrected size/unit parser
    ...
    if size_match:
        ...
        unit_map = {
            'מל': 'ml', 'מ"ל': 'ml', 'גר': 'g', 'גרם': 'g', 'ג"ר': 'g',
            'מג': 'mg', 'מ"ג': 'mg', 'ליטר': 'l', 'ל': 'l', 'קג': 'kg', 'ק"ג': 'kg', 
            'יחידות': 'units', 'יח': 'units'
        }
...
def get_extracted_attributes(product_name: str, brand: str):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a highly accurate data extraction assistant. Your only output is a single, valid JSON object."},
...
def update_product_attributes(cursor, product_id, new_attributes):
    cursor.execute("UPDATE products SET attributes = attributes || %s WHERE product_id = %s;", (Json(new_attributes), product_id))
...
def main():
    ...
    if args.review_file:
        try:
            ...
            print(f"-> Logging results to '{args.review_file}'")
        except IOError as e:
            print(f"[ERROR] Could not open review file: {e}")
            return
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if not products:
                print("✅ All products are already enriched. Nothing to do.")
                return
            ...
            for i, (product_id, name, brand) in enumerate(products):
                ...
                if attributes:
                    ...
                    if csv_writer:
                        row = [
                            product_id, name, source,
                            attributes.get('product_type', ''),
                            attributes.get('variant', ''),
                            attributes.get('size_value', ''),
                            attributes.get('size_unit', ''),
    ...
    except (Exception, KeyboardInterrupt) as e: # Catch Ctrl+C
