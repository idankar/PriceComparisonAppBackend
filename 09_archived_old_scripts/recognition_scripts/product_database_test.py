#!/usr/bin/env python3
"""
Product Database Recognition and Retrieval Test
Creates HTML visualization of test results across the large product database
"""
...
class ProductDatabaseTester:
    """Test product recognition and retrieval across the database"""
    ...
    def select_test_products(self, num_products: int = 20) -> List[Dict]:
        ...
        # If we need more products, randomly select from remaining
        if len(test_products) < num_products:
            remaining_items = [item for item in valid_items if item not in test_products]
            additional_needed = num_products - len(test_products)
            if remaining_items:
                additional = random.sample(remaining_items, min(additional_needed, len(remaining_items)))
                test_products.extend(additional)
    ...
    def run_similarity_search(self, query_image: str, index_path: str, top_k: int = 5) -> List[Dict]:
        """Run similarity search for a query image"""
        try:
            return search_similar_products(query_image, index_path, top_k)
        except Exception as e:
    ...
    def generate_html_report(self, test_results: List[Dict], output_path: str = "product_database_test_results.html"):
...
<head>
    ...
    <title>Product Database Recognition Test Results</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <h1>🛒 Product Database Recognition Test Results</h1>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <span>Total Products Tested</span>
            </div>
            <div class="stat-card">
...
"""
            ...
            if ocr_result.get('words'):
                html_content += f"""
                        ...
                        <div class="ocr-words">
"""
                for word in ocr_result['words'][:20]:  # Limit to first 20 words
                    html_content += f'<span class="ocr-word">{word}</span>'
                
                if len(ocr_result['words']) > 20:
                    html_content += f'<span class="ocr-word">+{len(ocr_result["words"]) - 20} more...</span>'
                    ...
                    html_content += f"""
                            <div class="similarity-item">
                                ...
                                <div style="font-size: 0.8em; margin-top: 5px;">
                                    {sim_item['text_for_embedding'][:50]}{'...' if len(sim_item['text_for_embedding']) > 50 else ''}
                                </div>
                            </div>
"""
...
def run_comprehensive_test():
    """Run comprehensive product database test"""
    ...
    # Initialize tester
    tester = ProductDatabaseTester(training_data_path, image_directories)
    
    # Select test products
    print("Selecting test products...")
    test_products = tester.select_test_products(num_products=25)
    print(f"Selected {len(test_products)} test products")
    
    # Check if training index exists, create if not
    index_path = "training_index.json"
    if not os.path.exists(index_path):
        print("Creating training index...")
        create_training_index(training_data_path, image_directories)
    else:
        print("Using existing training index")
    ...
    for i, product in enumerate(test_products, 1):
        ...
        similarity_results = tester.run_similarity_search(product['local_image_path'], index_path, top_k=5)
