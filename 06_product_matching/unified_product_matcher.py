...
class UnifiedProductMatcher:
    ...
    def calculate_fuzzy_score(self, prod1: Dict, prod2: Dict) -> Tuple[float, Dict]:
        ...
        attrs1 = prod1.get('attributes') or {}
        attrs2 = prod2.get('attributes') or {}
        size1 = attrs1.get('size_value') if isinstance(attrs1, dict) else None
        size2 = attrs2.get('size_value') if isinstance(attrs2, dict) else None
        if size1 and size2:
            try:
    ...
    def match_by_llm(self, products_by_retailer: Dict, unmatched: Set[int], 
                     sample_size: int = 1000) -> List[MatchResult]:
        ...
        for i in range(0, len(unmatched_products), batch_size):
            ...
            try:
                ...
                # If the response has a 'matches' key, use that, otherwise assume it's the array
                llm_matches = parsed.get('matches', parsed) if isinstance(parsed, dict) else parsed
    ...
    def run_complete_matching(self):
        ...
        fuzzy_matches, fuzzy_matched = self.match_by_fuzzy(
            products_by_retailer, all_matched, threshold=0.75
        )
        ...
        if self.llm_client and remaining:
            llm_matches, llm_matched = self.match_by_llm(
                products_by_retailer, remaining, sample_size=1000
            )
            ...
            for product in products:
                if product['product_id'] not in matched:
                    ...
                    """, (
                        ...
                        product.get('brand'),
                        ['barcode', 'fuzzy', 'llm'] if self.llm_client else ['barcode', 'fuzzy']
                    ))
